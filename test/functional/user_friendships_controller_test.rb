  ...
  context "#new" do
    ...
    context "when logged in" do
      ...
      should "assign a user friendship with the user as current user" do
        get :new, friend_id: users(:jim)
        assert_equal assigns(:user_friendship).user, users(:tim)
      end
  ...
  context "#create" do
    ...
    context "when logged in" do
      ...
      context "with a valid friend_id" do
        setup do
          post :create, friend_id: users(:rena).id
        end

        should "assign a friend object" do
          assert_equal users(:rena), assigns(:friend)
        end

        should "assign a user_friendship object" do
          assert assigns(:user_friendship)
          assert_equal users(:tim), assigns(:user_friendship).user
          assert_equal users(:rena), assigns(:user_friendship).friend
        end
